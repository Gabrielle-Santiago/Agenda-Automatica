...
def validarHorario(data, horario_inicial, horario_final):
    ...
    if horario_inicial < inicio_trabalho or horario_final > fim_trabalho:
        raise ValidationError("Horário fora do período de funcionamento. Se o procedimento ultrapassar o horário de atendimento, não será agendado. Tente outra data.")
...
def validarPausa(data, horario_inicial, horario_final):
    ...
    if horario_inicial < pausa_fim and horario_final > pausa_inicio:
        raise ValidationError("O horário solicitado coincide com o intervalo da empresa. O tempo total do procedimento deve finalizar antes do intervalo. Tente a partir de 16:30.")
